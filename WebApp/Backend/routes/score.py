...
def get_scans_by_userid(user_id: int, token_data: dict = Depends(verify_token), db: Session = Depends(get_db)):
    scans = db.query(UsersScansResults).filter(UsersScansResults.user_id == user_id).all()
    return {"scans": scans}


@router.delete("/delete_scan_by_scanid/{scan_id}")
def delete_scan_by_scanid(scan_id: int, token_data: dict = Depends(verify_token), db: Session = Depends(get_db)):
...
def get_pii_details_by_scanid(scan_id: int, token_data: dict = Depends(verify_token), db: Session = Depends(get_db)):
...
def get_feedbacks(token_data: dict = Depends(verify_token), db: Session = Depends(get_db)):
    feedback = db.query(PiiFeedbacks).all()
    return {"feedback": feedback}


@router.get("/get_score_by_scanid/{scan_id}")
def get_score_by_scanid(scan_id: int, token_data: dict = Depends(verify_token), db: Session = Depends(get_db)):
